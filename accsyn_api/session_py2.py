            ...
            if isinstance(d, dict):
                for key in d.keys():
                    if isinstance(d[key], dict):
                        ...
                        if re.match(
                            "^[0-9]{2,4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}$", str(
                            ...
                            if len(d[key].split("-")[0]) == 4:
                                ...
                                d[key] = datetime.datetime.strptime(
                                    d[key], "%y-%m-%dT%H:%M:%S")
                        # With millis
                        elif re.match("^[0-9]{2,4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}.[0-9]{3}$", str(Session.safely_printable(d[key]))):
                            if len(d[key].split("-")[0]) == 4:
                                d[key] = datetime.datetime.strptime(
                        ...
                        # DEPRECATED
                            ...
                            d[key] = datetime.datetime.strptime(
                                s, "%Y%m%d %H:%M:%S" if s.find("+") == -1 else "%Y%m%d+%H:%M:%S")
            return d
...
class Session(object):
    ...
    def __init__(
        ...
        for key in os.environ:
            if key.startswith("FILMHUB_"):
                Session.warning(
                    "Found old FilmHUB product environment variable '%s', please migrate!" %
                    key)
        if domain is None:
            if not (
                    'ACCSYN_DOMAIN' in os.environ or 'ACCSYN_ORG' in os.environ or 'FILMHUB_DOMAIN' in os.environ or 'FILMHUB_ORG' in os.environ):
                raise AccsynException(
                    "Please supply your Accsyn domain/organization or set ACCSYN_DOMAIN environment!")
        self._domain = domain or (
            os.environ['ACCSYN_DOMAIN'] if 'ACCSYN_DOMAIN' in os.environ else os.environ.get(
                'ACCSYN_ORG', os.environ.get(
                    'FILMHUB_DOMAIN', os.environ.get('FILMHUB_ORG'))))
        if username is None:
            if not (
                    'ACCSYN_API_USER' in os.environ or 'FILMHUB_API_USER' in os.environ):
                raise AccsynException(
        ...
        if 0 < len(session_key or ""):
            ...
            if 0 < len(pwd or ""):
                ...
                raise AccsynException(
                    "Please supply your Accsyn API KEY or set ACCSYN_API_KEY environment!")
    ...
    def login(self, revive_session_key=None):
        ...
        if self._api_key:
            ...
            headers = {
                "Authorization": "ASSession %s" %
                Session.base64_encode(
                    '{"domain":"%s","username":"%s","session_key":"%s"}' %
                    (self._domain, self._username, self._session_key_provided))}
        ...
        if data is not None and 0 < len(data):
            ...
            if (DEFAULT_EVENT_PAYLOAD_COMPRESS_SIZE_TRESHOLD < size):
                ...
                self.verbose("Compressed event payload %d>%d(%s%%)" % (
                    size, len(event['gz_data']), (100 * len(event['gz_data']) / size)))
    ...
    def rest(
        ...
        if proxy_type == "accsyn":
            ...
            try:
                ...
                Session.warning(
                    "Python lacks SOCKS support, please install 'pysocks' and try again...")
                raise ie
        elif proxy_type is not None:
            raise AccsynException("Unknown proxy type '%s'!" % proxy_type)
        url = "http%s://%s:%d/api/v1.0%s" % ("s" if ssl else "",
                                             hostname,
                                             port,
        ...
        if headers is None:
            if uri.find("registry/") != 0:
                assert (self._session_key is not None), (
                    "Need to be authenticated when communicating with Accsyn!")
                headers = {
                    'Authorization': "ASSession %s" % (base64.b64encode(
                        '{"domain":"%s","username":"%s","session_key":"%s"}' % (self._domain,
                                                                                self._username,
                                                                                self._session_key)))}
            else:
                headers = {}
        headers['ASDevice'] = "PythonAPI v%s @ %s %s(%s)" % (
        ...
        for iteration in range(0, 2):
            ...
            try:
                self.verbose(
                    "REST %s %s, data: %s" %
                    (method, url, data if not self._pretty_json else Session.str(data)))
                if method.lower() == "get":
                    r = requests.get(
                        url, params=urllib.quote(
                            Session.safe_dumps(data)), timeout=(
                            CONNECT_TO, READ_TO), verify=False, headers=headers)
                elif method.lower() == "put":
                    r = requests.put(
                        url, Session.safe_dumps(data), timeout=(
                            CONNECT_TO, READ_TO), verify=False, headers=headers)
                ...
                if timeout <= 0 or True:
                    raise AccsynException(
                        "Could not reach %s:%d! Make sure cloud server(%s) can be reached from you location and no firewall is blocking outgoing traffic at port %s. Details: %s" %
                        (hostname, port, port, hostname, traceback.format_exc() if not quiet else "(quiet)"))

                Session.warning(
                    "Could not reach %s:%d! Waited %ds/%s, will try again in %ds... Details: %s" %
                    (hostname,
                     port,
                     initial_timeout -
                     timeout,
            ...
            try:
                ...
                if not quiet:
                    self.verbose(
                        "%s/%s REST %s result: %s (~%sms)" %
                        (hostname, uri, method, Session.obscure_dict_string(
                            Session.safely_printable(
                                str(retval) if not self._pretty_json else Session.str(retval)).replace(
                                "'", '"')), t_start - t_end + 1))
                do_retry = False
                if not retval.get('message') is None:
                    ...
                    if retval.get('session_expired') is True:
                        if self._api_key is not None:
                            ...
                            self.info(
                                "Authenticated using API KEY and reused expired session...")
                            do_retry = True
                    if not do_retry:
                ...
                message = "The %s:%d/%s REST %s %s operation failed! Details: %s %s" % (hostname, port, uri, method, Session.obscure_dict_string(
                    Session.safely_printable(str(data)).replace("'", '"')), r.text, traceback.format_exc())
                Session.warning(message)
                raise AccsynException(message)
        if 'exception' in retval:
            message = "%s caused an exception! Please contact %s admin for more further support." % (
                uri, self._domain)
            Session.warning(message)
    ...
    def decode_query(self, query):
        ...
        assert (len(parts) == 1 or 3 <= len(
            parts)), ("Query has invalid syntax; statements can either be single ('<entity>') or with a WHERE statement ('<(sub)entity> WHERE {<entity>.}id=..{ AND ..}')")
        if len(parts) == 1:
            ...
            assert (parts[1].strip().lower() == "where"), (
                "Invalid query '%s', should be on the form '<entitytype> where <expression>'.")
            # Decode expression
            return {'entitytype': parts[0].lower(),
    ...
    def find(
        ...
        assert (
            0 < len(
                query or "") and (
                isinstance(
                    query, str) or isinstance(
                    query, str) or isinstance(
                        query, unicode))), ("Invalid query type supplied, must be of string type!")
        retval = None
    ...
    def update_one(self, entitytype, entityid, data):
        ''' Update an entity '''
        assert (0 < len(entitytype or "") and (isinstance(entitytype, str) or isinstance(entitytype, unicode)))\
            , ("Invalid entity type supplied, must be of string type!")
        entitytype = entitytype.lower().strip()
        assert (0 < len(entityid or "") and (isinstance(entityid, str) or isinstance(entityid, unicode)))\
    ...
    def ls(
        ...
        assert (
            0 < len(
                ...
                isinstance(
                    p, str) or isinstance(
                    p, unicode) or isinstance(
                        p, dict) or isinstance(
                            p, list))), ("No path supplied, or not a string/list!")
        data = {
            'op': "ls",
            'path': p,
            'download': True,
